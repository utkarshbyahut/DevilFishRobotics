...
Dark Spot Detector
==================
Masks everything that is not white or light, segments dark regions
as individual objects, and counts them.

Good for laminated printed crab images where crabs are dark
against a white or light background sheet.

Usage:
  python dark_spot_detector.py
  python dark_spot_detector.py --camera 1
  python dark_spot_detector.py --image crab.jpg
  python dark_spot_detector.py --calibrate
"""
...
def build_dark_mask(frame: np.ndarray, threshold: int, closing_radius: int) -> np.ndarray:
    ...
    2. Threshold: pixels darker than `threshold` become foreground
    3. Morphological close to merge fragmented blobs
    4. Morphological open to remove isolated noise
    """
...
def detect_objects(mask: np.ndarray, min_area: int) -> list[dict]:
    ...
    objects.sort(key=lambda obj: (obj["cy"] // 80, obj["cx"]))
...
def draw_results(frame: np.ndarray, mask: np.ndarray, objects: list[dict]) -> np.ndarray:
    """Overlay mask, boxes, IDs, and a count panel on the frame."""
    output = frame.copy()
    ...
    px, py, line_height = 10, 10, 22
    panel_height = 14 + line_height * len(panel_text)
    cv2.rectangle(output, (px, py), (px + 260, py + panel_height), (20, 20, 20), -1)
    cv2.rectangle(output, (px, py), (px + 260, py + panel_height), (200, 200, 200), 1)
    for index, text in enumerate(panel_text):
        color = (80, 240, 80) if index == 0 else (180, 180, 180)
        weight = FONT_THICK if index == 0 else 1
        cv2.putText(
            output,
            text,
            (px + 8, py + 16 + line_height * index),
            FONT,
            0.5,
            color,
            weight,
...
def calibrate_mode(cap: cv2.VideoCapture) -> None:
    """Live threshold tuner for the dark-mask prototype."""
    ...
    cv2.namedWindow(win)
    cv2.createTrackbar("Threshold", win, BRIGHTNESS_THRESHOLD, 255, lambda value: None)
    cv2.createTrackbar("Close radius", win, CLOSING_RADIUS, 40, lambda value: None)
    cv2.createTrackbar("Min area /10", win, MIN_BLOB_AREA // 10, 500, lambda value: None)
    ...
    while True:
        ...
        mask = build_dark_mask(frame, threshold, radius)
        objects = detect_objects(mask, max(min_area, 1))
        ...
        if cv2.waitKey(5) == 27:
            ...
            print(f"  BRIGHTNESS_THRESHOLD = {threshold}")
            print(f"  CLOSING_RADIUS       = {radius}")
            print(f"  MIN_BLOB_AREA        = {min_area}")
            break
...
def run_on_image(path: str, threshold: int, closing_radius: int, min_area: int) -> None:
    """Process a single image file and display or save the result."""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Dark Spot Detector - count dark objects on light background"
    )
    ...
    parser.add_argument(
        "--calibrate",
        action="store_true",
        help="Open live threshold tuning sliders",
    )
    parser.add_argument(
        "--threshold",
        type=int,
        default=BRIGHTNESS_THRESHOLD,
        help=f"Brightness cutoff for dark pixels (default {BRIGHTNESS_THRESHOLD})",
    )
    parser.add_argument(
        "--min-area",
        type=int,
        default=MIN_BLOB_AREA,
        dest="min_area",
